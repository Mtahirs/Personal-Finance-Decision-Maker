...
const attachFormListeners = (question) => {
    ...
    if (question.type === 'currency') {
        ...
        const button = currencySelect.querySelector<HTMLButtonElement>('button');
        const buttonText = button?.querySelector<HTMLSpanElement>('span');
        const dropdown = currencySelect.querySelector<HTMLElement>('.searchable-select-dropdown');
        const searchInput = currencySelect.querySelector<HTMLInputElement>('input[type="text"]');
        const list = currencySelect.querySelector<HTMLUListElement>('.searchable-select-list');
        ...
        document.addEventListener('click', (e) => {
            if (!currencySelect.contains(e.target as Node)) {
                dropdown.style.display = 'none';
                chevron.classList.remove('open');
            }
        });
    } else {
        const input = document.getElementById('question-input') as HTMLInputElement | HTMLSelectElement | null;
        if (!input) return;
